...
class RedisSession(object):
    def __init__(self, session_store, session_id=None, expire_seconds=None):
        self._store = session_store
        self._sid = session_id if session_id else self._store.generate_sid()
        self._dirty = False
        self.set_expires(expire_seconds)
...
class Session(object):
    def __init__(self, get_secure_cookie, set_secure_cookie, name='_session', expire_seconds=None):
        self.set_session = set_secure_cookie
        self.get_session = get_secure_cookie
        self.name = name
        self._expiry = expire_seconds
        self._dirty = False
        self.get_data()
    ...
    def save(self):
        if self._dirty:
            self.set_session(self.name, pickle.dumps(self._data), expire_seconds=self._expiry)
            self._dirty = False
