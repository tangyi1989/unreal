...
class ConnectionPool(object):
    ...
    """ 对 eventlet connection pool 的简单包装."""
    ...
    def get_connection(cls, **kwargs):
        """ 从连接池中获取连接，只有全部参数相同，才会使用同一个连接池。 """
        if getattr(cls, "__instance__", None) is None:
            pool = ConnectionPool()
            setattr(cls, "__instance__", pool)

        pool = getattr(cls, "__instance__")
        return pool.get_from_pools(**kwargs)
...
class Connection(object):
    ...
    def reconnect(self):
        ...
        self.close()
        self._db = ConnectionPool.get_connection(**self._db_args)
